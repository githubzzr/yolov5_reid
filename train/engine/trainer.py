...
from train.utils.reid_metric import R1_mAP
...
def do_train(
    ...
    checkpointer = ModelCheckpoint(output_dir, args.model_name, checkpoint_period, n_saved=10, require_empty=False)
    # ------------------------------------
    ...
    def log_validation_results(engine):
        if engine.state.epoch % eval_period == 0:
            ...
            #torch.save(state_dict,osp.join('logs',text+'.pth'))
            torch.save(state_dict,'logs/mAP_{:.1%}.pth'.format(mAP))
            #torch.save(state_dict,'logs/ckpt.pth')
            return cmc, mAP
